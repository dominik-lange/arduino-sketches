...
//VOLUME STORAGE
//max. 99 folders --> next is volume
const uint16_t EEPROM_VOLUME_KEY = 100; 

static void blink(uint8_t blinks, uint16_t blinkDelay) {
...
static void blinkButtonPressed() {
  blink(1, 200);
}
...
static void nextTrack(uint16_t track) {
  if (track == _lastTrackFinished) {
   ...
   if (knownCard == false)
    //if a new card is trained the end of track should not be processed
    return;
...
void setup() {
  ...
  // init DFPlayer Mini
  mp3.begin();
  int initialVolume = EEPROM.read(EEPROM_VOLUME_KEY);
  if(initialVolume == 255) 
    initialVolume = 15;
...
void loop() {
  do {
    mp3.loop();
    // Buttons arbe being handled with JS_Button --> every key can have 2 functions (short & long press)
    pauseButton.read();
    upButton.read();
    downButton.read();
    ...
    if (upButton.pressedFor(LONG_PRESS)) {
      ...
      mp3.increaseVolume();
      blinkButtonPressed();
      EEPROM.write(EEPROM_VOLUME_KEY, mp3.getVolume());
      ignoreUpButton = true;
